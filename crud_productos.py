...
def crear_producto(db: Session, producto: ProductoCreate):
    categoria = db.query(Categoria).filter(Categoria.id == producto.categoria_id, Categoria.activa == True).first()
    if not categoria:
...
def obtener_producto(db: Session, producto_id: int):
    return db.query(Producto).filter(Producto.id == producto_id, Producto.activa == True).first()
...
def restar_stock(db: Session, producto_id: int, cantidad: int):
    producto = db.query(Producto).filter(Producto.id == producto_id, Producto.activa == True).first()
